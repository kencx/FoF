...
def find_masses(data, estimator):
    ...
    for i, c in enumerate(data):
        if estimator == "virial":
            (
                ...
                vel_disp_err,
                projected_radius,
            ) = virial_mass_estimator(c)

            absmag = c.galaxies[:, 5]
            total_absmag = -2.5 * np.log10(
                sum(10 ** (-0.4 * absmag))
            )  # total absolute magnitude
            new_c = CleanedCluster(
                c.center_attributes,
                c.galaxies,
                mass,
                mass_err,
        ...
        if estimator == "projected":
            ...
            new_c = CleanedCluster(
                c.center_attributes, c.galaxies, mass, mass_err, None, None, None, None
            )
