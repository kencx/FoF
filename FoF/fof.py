...
# --- search algorithms ---
def candidate_center_search(galaxy_data, max_velocity, fname, path='FoF\\analysis\\derived_datasets\\'):
    '''
    ...
    for i, galaxy in enumerate(main_arr): 
        ...
        vel_bin = main_arr[abs(redshift_to_velocity(galaxy_arr[:,2], galaxy[2])) <= max_velocity] # select galaxies within appropriate redshift range

        if (len(vel_bin) >= 8):
            main_arr[i,-1] = find_number_count(galaxy, vel_bin[:,:2], 0.5*u.Mpc/u.littleh) # galaxies within 0.5 h^-1 Mpc

    candidate_centers = main_arr[main_arr[:,-1] >= 8] # select for galaxies with N(0.5) >= 8
    candidate_centers = candidate_centers[candidate_centers[:,-1].argsort()[::-1]] # sort by N(0.5) in desc
    assert candidate_centers[0,-1] == max(candidate_centers[:,-1]), 'Galaxy array is not sorted by N(0.5)'
...
def FoF(galaxy_data, candidate_centers, richness, overdensity, max_velocity=2000*u.km/u.s, linking_length_factor=0.1, virial_radius=1.5*u.Mpc/u.littleh):
    '''
    ...
    for i, center in enumerate(candidate_centers): # each row is a candidate center to search around
        ...
        if tracker[i]:
            ...
            ang_virial_radius = linear_to_angular_dist(virial_radius, center[2]).to('rad') # convert proper virial radius to angular separation
            ...
            max_dist = linear_to_angular_dist(max_dist, center[2]).value # convert comoving distance to angular separation

            virial_dist, virial_idx = virial_gsp.bubble_neighbors(np.array([center[:2]]), distance_upper_bound=max_dist) # center must be a ndarray of (n,2)
            virial_points = velocity_bin[tuple(virial_idx)] # convert to tuple for deprecation warning

            if len(virial_points) >= 12: # reject if <12 galaxies within virial radius (to save time) 
                mean_sep = mean_separation(len(virial_points), center[2], max_dist*u.degree, max_velocity, survey_area=1.7) # Mpc
    ...
    candidate_clusters = np.array([[c.ra, c.dec, c.z, c.gal_id] for c in candidates]) # get specific attributes from candidate center sample
    ...
    for center in candidates:
        ...
        velocity_bin = candidate_clusters[abs(redshift_to_velocity(candidate_clusters[:,2], center.z)) <= max_velocity] # select galaxies within max velocity
        ...
        c_dist, c_idx = center_gsp.bubble_neighbors(np.array([c_coords]), distance_upper_bound=max_dist) # center must be a ndarray of (n,2)
    ...
    for center in merged_candidates:
        ...
        c_dist, c_idx = cluster_gsp.bubble_neighbors(np.array([c_coords]), distance_upper_bound=max_dist) # center must be a ndarray of (n,2)
        bcg_arr = center.galaxies[tuple(c_idx)]

        if len(bcg_arr) and len(center.galaxies): # check for galaxies within 0.25*virial radius

            mag_sort = bcg_arr[bcg_arr[:,3].argsort()] # sort selected galaxies by abs mag (brightness)
            mask = np.isin(mag_sort[:,4], bcg_space_gal_id, invert=True) # filter for galaxies that are not existing centers
            ...
            if len(mag_sort):
                ...
                if (abs(bcg[3]) > abs(center.bcg_absMag)) and (bcg[4] != center.gal_id):
                    ...
                    bcg_clusters = np.delete(bcg_clusters, np.where([c.gal_id for c in bcg_clusters] == center.gal_id))
                    bcg_clusters = np.concatenate((bcg_clusters, np.array([new_cluster]))) # add new center to array


    bcg_clusters = np.array([c for c in bcg_clusters if c.richness >= richness]) # select only clusters >= richness
    final_clusters = []
