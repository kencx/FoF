...
def compare_clusters(sample, catalog, matching_distance, richness_plot=True):
    ...
    sample = sample[(sample[:,2] >= min_redshift-0.1) & (sample[:,2]<= max_redshift+0.1)]
    catalog = catalog[(catalog[:,2] >= min_redshift-0.1) & (catalog[:,2]<= max_redshift+0.1)]
    logging.info(f'No. of galaxies in sample: {len(sample)} and No. of galaxies in catalog: {len(catalog)}')
    ...
    for i, c in enumerate(catalog): # for each cluster in the catalog, find matching sample clusters
        ...
        if len(sample_coord):
            ...
            idx, d2d, _ = match_coordinates_sky(catalog_coord, sample_coord) # find nearest match, idx is index of sample
            if d2d <= max_sep:
    ...
    logging.info(f'{count}/{len(catalog)} matched. {count/len(catalog)*100} % of candidates matched')
...
if __name__ == "__main__":
    ...
    ultra_deep_matched, ultra_catalog_idx = compare_clusters(virial_clusters, ultra_deep_arr, 0.5*u.Mpc/u.littleh, richness_plot=False)
