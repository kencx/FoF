...
from params import *
from sdss_cluster import Cluster
...
def setdiff2d(arr_1, arr_2): # finds common rows between two 2d arrays and return the rows of arr1 not present in arr2
    ...
    dtype = [('ra', np.float64), ('dec', np.float64), ('redshift', np.float64), ('z_unc', np.float64), ('appMag', np.float64), ('ID', np.float64), ('absMag', np.float64), ('N(0.5)', np.float64)]
...
# --- search algorithms ---
def candidate_cluster_search(galaxy_data, max_velocity, path):
    '''
    ...
    2 arrays are returned and saved into the path.
    ...
    np.savetxt(path+'luminous_galaxy_data.csv', luminous_galaxy, delimiter=',')
    ...
    for i, center in enumerate(luminous_data): # each row is a candidate center to search around
        ...
        if tracker[i]:
            ...
            if len(virial_points) >= 12: # reject if less than 12 surrounding galaxies within virial radius (to save time) 
                mean_sep = mean_separation(len(virial_points), center[2], max_dist*u.degree, max_velocity, np.sqrt(31637)) # Mpc
            ...
            # if len(candidates) >= 500: # for quick testing
            #     break
    ...
    for i in range(1, len(bins)+1):
        ...
        while (size_change != 0) and (len(bin_galaxies) > 0): # repeat until convergence for each bin
            ...
            if len(bin_galaxies) > 2:
                ...
                if (bin_galaxies[-1,-1] == 0.0) & (bin_galaxies[-1,-2] == 0.0): # if galaxy with largest deviation is center
                ...
                assert sum(ignored_galaxy[-2:] == 0.0) != 2, 'Galaxy with largest deviation is the cluster center'
...
def flag_plots(clusters):
    ...
    bins = np.arange(0.02, 0.32,0.00666)
...
if __name__ == "__main__":
    ...
    #############################
    # main_arr, luminous_arr = candidate_cluster_search(df, max_velocity=max_velocity)
